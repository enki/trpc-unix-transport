 ...
 */
export function unixSocketLink<TRouter extends AnyRouter>(
  ...
  return () => {
    return ({ op }) => {
      return observable((observer) => {
        ...
        let transport: LengthPrefixedTransport | null = null;
        ...
        transport.on('message', (message: string) => {
          try {
            ...
            if (response.error) {
              observer.error(
                new TRPCClientError(response.error.message || 'Unknown error')
              );
