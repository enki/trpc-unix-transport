...
export class TRPCUnixServer {
    ...
    async start() {
        ...
        try {
            ...
            // Cleanup on failure
            await this.cleanup();
            throw new Error(`Unix socket server failed to start: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async stop() {
     ...
     */
    async testSocketConnection(timeoutMs) {
    ...
    async preflightChecks() {
        ...
        try {
            const stats = await this.withTimeout(fs.stat(socketDir), 5000, 'stat socket directory');
            this.logger.debug({
                socketDir,
        ...
        try {
            ...
            if (isLive) {
                throw new Error(`Socket file exists and appears to be in use: ${this.socketPath}. ` +
    ...
    async createSocket() {
        ...
        await this.withTimeout(fs.mkdir(socketDir, { recursive: true }), 5000, 'mkdir socket directory');
        this.logger.debug('Socket directory created/verified');
        // Step 2: Clean up existing socket (with timeout)
        try {
            await this.withTimeout(fs.unlink(this.socketPath), 5000, 'unlink existing socket');
        ...
        await new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                ...
                reject(new Error(`server.listen() callback did not fire after ${timeoutMs}ms. ` +
        ...
        await this.withTimeout(fs.chmod(this.socketPath, 0o600), 2000, 'chmod socket file');
    ...
    async verifySocket() {
        ...
        if (!fileFound) {
            throw new Error(`Socket file never became visible after ${maxRetries * 100}ms. ` +
    ...
    async testSocketEcho() {
        return new Promise((resolve, reject) => {
            ...
            transport.once('message', (response) => {
                clearTimeout(timeout);
                try {
     ...
     */
    async handleTRPCRequest(transport, request, socket) {
        const { id } = request;
        ...
        const routerWithConfig = this.options.router;
        const transformer = routerWithConfig._def._config.transformer;
        const input = transformer?.input && serializedInput !== undefined
        ...
        try {
            ...
            const errorDataRecord = errorData && typeof errorData === 'object' && !Array.isArray(errorData)
                ? errorData
                : undefined;
            const errorDataObject = errorDataRecord ?? {};
            await respond({
                id,
                error: {
                    message: error.message,
...
export function createTRPCUnixServer(options, logger) {
